    ...
    return (
      <main className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center px-4">
        <div className="max-w-md text-center">
          ...
          <h1 className="text-xl font-semibold text-slate-100">Plan unavailable</h1>
          <p className="mt-2 text-sm text-slate-400">{error || 'This onboarding plan could not be found.'}</p>
        </div>
      </main>
    )
  }
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100">
      <header className="border-b border-slate-800 bg-slate-900/60">
        <div className="mx-auto flex max-w-4xl items-center gap-3 px-6 py-5">
          ...
          <div>
            <div className="text-sm font-semibold text-slate-100">{data.plan.title || 'Onboarding Plan'}</div>
            <div className="text-xs text-slate-500">Shared by your customer success team</div>
          </div>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-4xl px-6 py-10">
        <section className="rounded-2xl border border-slate-800 bg-slate-900 p-7">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold tracking-tight text-slate-100">{accountName}</h1>
              <p className="mt-1 text-sm text-slate-400">Your path to first value and go-live.</p>
            ...
            <div className="text-right">
              <div className="text-4xl font-extrabold tabular-nums text-teal-300">{pct}%</div>
          ...
          <div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="rounded-xl border border-slate-800 bg-slate-950/40 px-4 py-3">
              <div className="text-xs uppercase tracking-wide text-slate-500">First value milestone</div>
              <div className="mt-1 text-sm font-medium text-slate-200">{firstValue?.name || 'Not defined'}</div>
              {firstValue && (
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-950/40 px-4 py-3">
              <div className="text-xs uppercase tracking-wide text-slate-500">Go-live milestone</div>
              <div className="mt-1 text-sm font-medium text-slate-200">{goLive?.name || 'Not defined'}</div>
              {goLive && (
                <div className="mt-1 text-xs text-slate-500">
        ...
        <section className="mt-8">
          <h2 className="mb-4 text-sm font-semibold uppercase tracking-wide text-slate-400">Milestones</h2>
          {milestones.length === 0 ? (
            <div className="rounded-xl border border-dashed border-slate-800 bg-slate-900/40 px-6 py-12 text-center text-sm text-slate-400">
                  ...
                  <li key={m.id} className="relative">
                    <span
                      className={`absolute -left-[31px] top-2 h-3.5 w-3.5 rounded-full border-2 ${
                        done ? 'border-teal-400 bg-teal-400' : (m.status || '').toLowerCase() === 'blocked' ? 'border-rose-400 bg-rose-500/40' : 'border-slate-600 bg-slate-900'
                      }`}
                    />
                    <div className="rounded-xl border border-slate-800 bg-slate-900 px-4 py-3">
                      ...
                      <div className="mt-2 flex flex-wrap gap-x-6 gap-y-1 text-xs text-slate-500">
                        ...
                        {m.completed_at && <span className="text-teal-400">Completed {fmtDate(m.completed_at)}</span>}
                      </div>
                    </div>
                  </li>
                )
